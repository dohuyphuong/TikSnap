# Consuming Quick Mark Design System in web apps

Read `artifacts/quick-mark-system/docs/AGENTS.md` first. This guide covers
...
`artifacts/quick-mark-system/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/quick-mark-system/styles.css";
```
...
`node_modules/@workspace/quick-mark-system/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/quick-mark-system/components/ui/button";
import { cn } from "@workspace/quick-mark-system/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/quick-mark-system/hooks/use-toast";
```
...
`@workspace/quick-mark-system/components/ui/button`. Run the app's typecheck
