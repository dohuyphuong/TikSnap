# Migrating Expo UI to Quick Mark Design System

Read `artifacts/quick-mark-system/docs/AGENTS.md` and
`artifacts/quick-mark-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/quick-mark-system/lib/native-theme` and
  `@workspace/quick-mark-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/quick-mark-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/quick-mark-system` and retains only
